...
export default class TaskCreateScreen extends Component {
  ...
  onNavigatorEvent(event) {
    if (event.type == 'NavBarButtonPress') {
      if (event.id == 'create_task_screen_home') {
  ...
  _onSubmit() {
    ...
    TaskDataService.createTask(task).then((responseJSON) => {
      this.setState({
        loading: true
      })
      this.props.navigator.pop()
      AppEventBus.trigger('task_created', responseJSON)
    }).catch((error) => {
      this.setState({
        loading: false
      })
